...
export default async function Home(){const user=await requireUser();const now=new Date();const [my,overdue,critical,events,incidents,projects]=await Promise.all([
 ...
 return <Shell><h2 className="text-3xl font-bold">Inicio</h2><div className="grid gap-4 md:grid-cols-3"><Stat label="Mis tareas pendientes" value={my}/><Stat label="Tareas vencidas" value={overdue}/><Stat label="Tareas críticas" value={critical}/><Stat label="Eventos próximos" value={events}/><Stat label="Incidencias abiertas" value={incidents}/><Stat label="Proyectos activos" value={projects}/></div><div className="card"><h3 className="font-semibold">Accesos rápidos</h3><div className="mt-3 flex flex-wrap gap-2"><a className="btn" href="/tasks#new">Nueva tarea</a><a className="btn" href="/events#new">Nuevo evento</a><a className="btn" href="/incidents#new">Nueva incidencia</a><a className="btn" href="/projects#new">Nuevo proyecto</a><a className="btn" href="/tasks?status=PENDIENTE">Ver tareas vencidas</a><a className="btn" href="/incidents?status=PENDIENTE">Ver incidencias abiertas</a></div></div><div className="card"><h3 className="font-semibold">Filtros disponibles</h3><p className="mt-2 text-sm text-slate-600">Las vistas permiten filtrar por área, estado, prioridad, responsable y búsqueda de texto.</p></div></Shell>}
