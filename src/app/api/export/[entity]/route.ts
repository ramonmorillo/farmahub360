...
function csv(rows:any[]){ const head=['id','titulo','area','responsable','prioridad','estado','visibilidad','creado']; return [head.join(','),...rows.map(r=>[r.id,r.title??r.name,r.area?.name,r.responsible?.name,(priorityLabels as Record<string,string>)[r.priority]??r.priority,(statusLabels as Record<string,string>)[r.status]??r.status,(visibilityLabels as Record<string,string>)[r.visibility]??r.visibility,r.createdAt].map(v=>`"${String(v??'').replaceAll('"','""')}"`).join(','))].join('\n') }
