...
type ListedUser = { id?: string; userId?: string };
export type SecureRecord = {
  visibility: Visibility;
  areaId?: string | null;
  createdById: string;
  responsibleId?: string | null;
  assignees?: ListedUser[];
  participants?: ListedUser[];
  customVisibility?: ListedUser[];
};

export const leadershipRoles: Role[] = ['ADMIN', 'JEFATURA'];

export function canManageUsers(user: Pick<CurrentUser, 'role'>) {
...
export function canViewRecord(user: CurrentUser, record: SecureRecord) {
  ...
  const listed = [
    ...(record.assignees ?? []),
    ...(record.participants ?? []),
    ...(record.customVisibility ?? []),
  ].some((item) => item.id === user.id || item.userId === user.id);
